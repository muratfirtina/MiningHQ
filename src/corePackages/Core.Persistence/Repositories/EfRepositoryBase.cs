    ...
    where TContext : DbContext
{
    ...
    {
        // Handle special case for getting all data (-1 values)
        if (index == -1 || size == -1)
        {
            var allItems = await GetAllByDynamicAsync(dynamic, predicate, include, index, size, withDeleted, enableTracking, cancellationToken);
